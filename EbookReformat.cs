...
public class EbookReformat
{
    ...
    private static string ConvertLine(string s)
    {
        while (s.Contains("_image:"))
        {
            ...
            s = string.Concat(s.AsSpan(0, pos0), "[image]", s.AsSpan(pos1 + 1));
        ...
        {
            newResult = newResult[0..^4].TrimEnd() + "</i>";
        }
    ...
    private static string ExpandHexChars(string s)
    {
        ...
        {
            int posStart = s.IndexOf("&#", posEnd + 1);
            result.Append(s.AsSpan(posEnd + 1, posStart - posEnd - 1));
